...
import aaa from './agora.js';

let leaveAndRemoveLocalStream = aaa.leaveAndRemoveLocalStream;
let joinStream = aaa.joinStream;
...
// // Détection de la proximité entre les joueurs
function arePlayersClose(player1, player2, distanceThreshold) {
...
// Appeler cette fonction à chaque mise à jour de la scène Phaser
...
/** @type {Array<[Player, number]>} */
  ...
  constructor() {
    ...
    this.SelfId = null;
    this.#moveDelay = 38; // Délai en millisecondes pour actualiser le mouvement
    this.#lastMoveTime = 0;
    this.#moveDelay_s = 30;
...
// #endregion
  ...
  update(time) {
    let stream = false;
    const selfp = this.players.find(p => p.id === this.SelfId);
    // for (const play of this.PLAYERS) {
    //   const player = play[0];
    //     const p = this.players.find(p => p.id === play[1]);
    //     console.log(p.encounters);
    //     const encount = p.encounters.indexOf(1);
    //     console.log(encount);
    //     if (encount !== -1) {
    //       joinStream(`reunion_${encount}`);
    //       console.log(`success reunion_${encount}`);
    //     }
    //     else {
    //       if (play[1] !== this.SelfId && arePlayersClose(this.#player.sprite, player.sprite, 130) && player.sprite.y > 18*TILE_SIZE && this.#player.sprite.y > 18*TILE_SIZE) {
    //         stream = true;
    //         joinStream("common");
    //         break;
    //         }
    ...
    for (const play of this.PLAYERS) {
        ...
        if (encount === -1) {
          ...
          if (encount !== -1 && selfp.encounters.indexOf(1) === encount) {
            stream = true;
            joinStream();
            break;
          }
    ...
    socket.on('players', (serverPlayers) => {
        ...
        // PlayerObj.sprite.x = playr.x;
        // PlayerObj.sprite.y = playr.y; 
    ...
    //   if (playr[1] !== this.SelfId) {
    //     const P = this.players.find(p => p.id === playr[1]);
    //   playr[0].sprite.x = P.x;
    //   playr[0].sprite.y = P.y;
    //   }
